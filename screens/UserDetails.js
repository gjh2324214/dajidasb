...
const UserDetails = ({ route, navigation }) => {
  ...
  const dispatch = useDispatch();
    ...
    return (
        <View style={styles.container}>
          <View style={styles.header}>
            ...
            <Text style={styles.eventCategory}>phone：{event.phone}</Text>
          </View>
          <View style={styles.infoContainer}>
            <Text style={styles.infoLabel}>email:</Text>
            <Text style={styles.infoValue}>{event.email}</Text>
          </View>
          <View style={styles.infoContainer}>
             <Text style={styles.infoLabel}>sex:</Text>
             <Text style={styles.infoValue}>{event.sex}</Text>
           </View>
        </View>
      );
    }
    ...
    if(level == 0) {
        const [userName, setUserName] = useState(event.userName)
        const [phone, setPhone] = useState(event.phone)
        const [email, setEmail] = useState(event.email)
        const [sex, setSex] = useState(event.sex)
        const id = event.id
        const handleUpdate = () => {
                 api.post('/user/update', {id,userName,phone,email,sex})
            ...
            const handleDelete = () => {
                api.delete('/user/delete?id=' + event.id)
                      .then(async function(data){
        ...
        return (
            <View style={styles.container2}>
              <View style={styles.infoContainer2}>
                  <Text style={styles.infoLabel2}>username:</Text>
                  <TextInput
                          style={styles.input2}
        ...
        <View style={styles.infoContainer2}>
                  <Text style={styles.infoLabel2}>phone:</Text>
                  <TextInput
                          style={styles.input2}
                          placeholder="phone"
                          value={phone}
        ...
        <View style={styles.infoContainer2}>
                  <Text style={styles.infoLabel2}>email:</Text>
                  <TextInput
                          style={styles.input2}
                          placeholder="email"
                          value={email}
        ...
        <View style={styles.infoContainer2}>
                  <Text style={styles.infoLabel2}>sex:</Text>
                  <TextInput
                          style={styles.input2}
                          placeholder="sex"
                          value={sex}
